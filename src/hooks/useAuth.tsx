...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Update last_active_at when user is active (React Native version)
  useEffect(() => {
    if (!user?.id) return;
    ...
    // Update on app state changes (foreground/background)
    const handleAppStateChange = (nextAppState: AppStateStatus) => {
      const now = Date.now();
      if (nextAppState === 'active' && (now - lastUpdate > 60 * 1000)) {
        ProfileService.updateLastActive(user.id);
        lastUpdate = now;
      }
    };
  ...
  const sendPasswordResetEmail = async (email: string) => {
    try {
      // For mobile, we'll use the deep link configuration
      const { error } = await supabase.auth.resetPasswordForEmail(email, { 
        redirectTo: 'spice://reset-password' 
      });
