...
import { apiGenerateBio } from '../services/api';
...
export const ProfileSetupPage: React.FC = () => {
  ...
  const handleGenerateBio = async () => {
    if (formData.interests.length < 3) {
      setValidationError('Select at least 3 interests to generate a bio.');
      return;
    }
    setValidationError(null);
    ...
    try {
        ...
        const { bio } = await apiGenerateBio(formData.interests, token);
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center mb-2">
                    <Label htmlFor="bio" className="text-white mb-0">Bio</Label>
                    <Button
                        onClick={handleGenerateBio}
                        disabled={isGeneratingBio || formData.interests.length < 3}
                ...
                <Textarea id="bio" value={formData.bio} onChange={(e) => setFormData({ ...formData, bio: e.target.value })} placeholder="Tell us about yourself, or generate one with AI based on your interests!" rows={4} className={inputClasses} />
