      ...
      <div className="max-w-2xl mx-auto p-8 bg-base-200 rounded-xl shadow-lg animate-fade-in">
        <div className="flex flex-col items-center text-center">
          {photos && photos.length > 0 ? (
            ...
            <div
              className="w-40 h-40 rounded-full bg-brand-secondary flex items-center justify-center mb-6 border-4 border-brand-primary"
          ...
          <p className="text-md text-text-secondary">
            {age ? `${age}` : 'Age N/A'} - {location || 'Location N/A'}
          </p>
        </div>
        ...
        <div>
          <h3 className="text-xl font-semibold text-brand-secondary mb-4 text-center">
            Your Interests:
          </h3>
          <div className="flex flex-wrap justify-center gap-2">
            {interests.length > 0 ? (
              interests.map((interest) => (
                <span
                  key={interest}
                  className="bg-base-300 text-brand-secondary text-sm font-medium px-3 py-1 rounded-full"
                >
                  {interest}
                </span>
              ))
            ) : (
              <p className="text-text-secondary">No interests added yet.</p>
            )}
          </div>
        </div>
      </div>
