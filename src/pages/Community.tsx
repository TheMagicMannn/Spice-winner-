...
// Full Screen Dialog Component for Mobile
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className="fixed inset-0 z-50 bg-black text-white overflow-y-auto data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 sm:inset-4 sm:rounded-lg sm:border sm:border-pink-500/30"
      {...props}
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 z-50 rounded-full p-2 bg-black/80 border border-pink-500/50 opacity-70 transition-opacity hover:opacity-100 focus:outline-none">
...
function ISOPostDetailModal({ post, isOpen, onClose }: { post: ISOPost | null; isOpen: boolean; onClose: () => void }) {
  ...
  const [selectedUser, setSelectedUser] = useState<ISOUser | null>(null);
  const [showUserProfile, setShowUserProfile] = useState(false);

  if (!post) return null;

  const handleUserClick = (user: ISOUser) => {
    setSelectedUser(user);
    setShowUserProfile(true);
  };
                  ...
                  <div className="flex flex-wrap gap-3">
                    {post.likedBy.map((user) => (
                      <button
                        ...
                        className="flex items-center space-x-2 bg-black/40 rounded-full pr-4 hover:bg-black/60 transition-all"
                      >
                        <img
                          src={user.image}
                          alt={user.name}
                        ...
                        {user.isVerified && <Shield className="h-4 w-4 text-blue-400" />}
                      </button>
                    ))}
                  </div>
                </div>
                        ...
                        <div className="flex items-start space-x-3">
                          <button onClick={() => handleUserClick(comment.user)}>
                            <img
                              ...
                              className="w-12 h-12 rounded-full object-cover hover:ring-2 hover:ring-pink-400 transition-all"
                            />
                          </button>
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-2">
                              ...
                              {comment.user.isVerified && <Shield className="h-4 w-4 text-blue-400" />}
                            ...
                            <span className="text-white/50 text-sm mt-2 block">{comment.postedAt}</span>
                          </div>
                        </div>
                      </div>
                    ))}
