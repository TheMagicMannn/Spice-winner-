-- FIX_ACTIVITY_LOG_RLS_FINAL.sql
-- Updated version with safe activity types that match your database constraints
...
-- ============================================
-- STEP 6: GET ALLOWED ACTIVITY TYPES
-- ============================================

-- First, let's check what activity types are actually allowed
DO $$
DECLARE
    constraint_def TEXT;
BEGIN
    ...
    RAISE NOTICE 'Allowed activity types: %', constraint_def;
END $$;
...
-- ============================================
-- STEP 8: CREATE TEST DATA FUNCTION (UPDATED)
-- ============================================
...
DECLARE
  ...
  v_user_id UUID;
  v_count INTEGER := 0;
  -- Use only basic activity types that are most likely to exist
...
BEGIN
  ...
  FOR v_days_ago IN 0..29 LOOP
    ...
    FOR i IN 1..(5 + floor(random() * 15)::int) LOOP
      ...
      v_activity_type := v_activity_types[1 + floor(random() * array_length(v_activity_types, 1))::int];
      
      -- Try to insert activity, skip if constraint violation
      BEGIN
        INSERT INTO user_activity_log (
          user_id,
          activity_type,
...
-- ============================================
-- STEP 9: CREATE DAILY REPORT FUNCTION
-- ============================================
...
BEGIN
  ...
  IF v_report_exists THEN
    ...
    SET
      total_signups = (SELECT COUNT(DISTINCT user_id) FROM user_activity_log WHERE activity_type IN ('signup', 'user_signup') AND DATE(created_at) = p_date),
      total_logins = (SELECT COUNT(*) FROM user_activity_log WHERE activity_type IN ('login', 'user_login') AND DATE(created_at) = p_date),
      total_messages = (SELECT COUNT(*) FROM user_activity_log WHERE activity_type IN ('message', 'message_sent') AND DATE(created_at) = p_date),
      total_likes = (SELECT COUNT(*) FROM user_activity_log WHERE activity_type IN ('like', 'profile_liked') AND DATE(created_at) = p_date),
      total_matches = (SELECT COUNT(*) FROM user_activity_log WHERE activity_type IN ('match', 'match_created') AND DATE(created_at) = p_date),
      total_payments = (SELECT COALESCE(SUM((activity_data->>'amount')::numeric), 0) FROM user_activity_log WHERE activity_type IN ('payment', 'payment_completed') AND DATE(created_at) = p_date),
  ...
  ELSE
    ...
    SELECT
      p_date,
      COUNT(DISTINCT CASE WHEN activity_type IN ('signup', 'user_signup') THEN user_id END),
      COUNT(CASE WHEN activity_type IN ('login', 'user_login') THEN 1 END),
      COUNT(CASE WHEN activity_type IN ('message', 'message_sent') THEN 1 END),
      COUNT(CASE WHEN activity_type IN ('like', 'profile_liked') THEN 1 END),
      COUNT(CASE WHEN activity_type IN ('match', 'match_created') THEN 1 END),
      COALESCE(SUM(CASE WHEN activity_type IN ('payment', 'payment_completed') THEN (activity_data->>'amount')::numeric END), 0),
...
-- ============================================
-- STEP 10: SHOW ALLOWED ACTIVITY TYPES
-- ============================================

SELECT 
    'Check constraint definition:' as info,
    pg_get_constraintdef(oid) as allowed_activity_types
FROM pg_constraint
WHERE conrelid = 'public.user_activity_log'::regclass
AND conname LIKE '%activity_type%';

-- Success message
SELECT '✅ RLS policies fixed! The function will now only use allowed activity types. Run: SELECT populate_test_activity_data();' as status;
