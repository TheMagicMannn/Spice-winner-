      ...
      <View style={styles.content}>
        ...
        {profile.location && (
          <View style={styles.infoRow}>
            <MapPin color={spiceTheme.colors.primary} size={16} />
  ...
  const renderCompactCard = () => (
     <View style={[styles.card, styles.compactCard]}>
        ...
        <View style={styles.compactContent}>
            ...
            <View style={styles.infoRow}>
                <Calendar color={spiceTheme.colors.textSecondary} size={12} />
                <Text style={styles.compactInfo}>{profile.age || '?'} yrs</Text>
            </View>
            {profile.location && (
                 <View style={styles.infoRow}>
                    <MapPin color={spiceTheme.colors.textSecondary} size={12} />
...
const styles = StyleSheet.create({
  card: { ...themeStyles.card, margin: 0 },
  ...
  rightNav: { right: 8 },
  content: { padding: 16 },
  profileName: { ...themeStyles.title, ...themeStyles.textGradient, marginBottom: 8 },
  infoRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4, gap: 4 },
  infoText: { color: spiceTheme.colors.textSecondary, fontSize: 16 },
  sectionTitle: { ...themeStyles.title, fontSize: 18, marginTop: 16, marginBottom: 8 },
  bioText: { color: spiceTheme.colors.textSecondary, lineHeight: 22 },
  actions: { flexDirection: 'row', padding: 16, gap: 8 },
  messageButton: { ...themeStyles.buttonPrimary, flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  likeButton: { padding: 12, backgroundColor: spiceTheme.colors.border, borderRadius: 8 },
  buttonText: { ...themeStyles.buttonPrimaryText },
  compactCard: {flexDirection: 'row', padding: 8, alignItems: 'center', gap: 8},
  compactImage: {width: 64, height: 64, borderRadius: 8},
  compactContent: {flex: 1},
  compactName: { ...themeStyles.title, fontSize: 16 },
  compactInfo: {color: spiceTheme.colors.textSecondary, fontSize: 12}
});
